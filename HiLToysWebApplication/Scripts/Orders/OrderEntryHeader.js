...
var lineItemDisplay = function (companyName,shipperID) {
...
function IntializeLineItem(ishippers) {
    ...
    for (i = 0; i < shippers.length; i++) {
        var newLineItem = CreateLineItem(shippers[i]);
        viewModel.Shippers.push(newLineItem);
...
function CreateLineItem(LineItem) {
    ...
    lineItem.ShipperID = ko.observable(LineItem.ShipperID);
    lineItem.CompanyName = ko.observable(LineItem.CompanyName);
